const domain = 'https://cercle.business';
...
class MoniaWebComponent extends HTMLElement {
    ...
    async loadPageContent(shadowRoot, content) {
        if (path) {
            ...
            widgetWrapper.style.bottom = "calc(50px + env(safe-area-inset-bottom))";
            ...
            openIcon.addEventListener("click", async function () {
                ...
                widgetWrapper.style.bottom = "calc(40px + env(safe-area-inset-bottom))";
...
async chats() {
    this._contentLoaded.then(() => {
        ...
        const fileUrlPrefix = `${domain}/widgets/monia/chat_logs/` + member + `/1/`;
        ...
        try {
            this._contentLoaded.then(() => {
                ...
                if (variablesElement) {
                    ...
                    const actid_perms = localStorage.getItem(page + "_auths") || 1; 
            ...
            .then(data => {
                if (data.success === "true" && data.token && data.token.trim() !== '') {
                    ...
                    if (token && token.trim() !== '') {
						...
						const newURL = `${domain}/index.php?option=com_comprofiler&view=pluginclass&plugin=cbautoactions&action=action&actions=336&format=raw`;
