...
// 		console.log ('actid: ', actid);
	...
	//end SMS for non registered   
		...
		//test main agent JC
// 		if (actid === 11875){
// 					console.log('Starting MoniA test main agent...');
...
//NEW DEV PYTHON FASTAPI		
		if (actid === 11875){		
			console.log("starting python application call");
			try {
					const questionData = {
							question: question,
							envTools: envTools
...
// 						console.log('question data: ' , questionData);
					const response = await axios.post('http://localhost:4100/process_question', questionData);
					...
					res.send(response.data);
						...
						//if (!res.headersSent) {
								console.error('Erreur lors de la connexion à API Python:', error);
