...
class MoniaWebComponent extends HTMLElement {
    ...
    async loadPageContent(shadowRoot, content) {
        if (path) {
			...
			scriptElement.src = `${domain}/widgets/cercle/js/oboe-browser.min.js`;
...
loadOboe() {
    return new Promise((resolve, reject) => {
        ...
        oboeScript.src = `${domain}/widgets/cercle/js/oboe-browser.min.js`;
...
async setupOboe(aiurl, params) {
	const tokenRegex = /token=([^&]*)/;
	const tokenMatch = path.match(tokenRegex);
	const token = tokenMatch ? tokenMatch[1] : null;
    const url = `${aiurl}?${new URLSearchParams(params)}`;

    const thinkingElement = this.getThinkingValue();
    const thinkingDiv = this.shadowRoot.getElementById('thinking');
    if (thinkingElement) {
...
// 			let token = dataObject.token; 
			...
			// Si nous ne construisons pas une valeur
				...
				tokBuffer += token;
				buildingKeyword += token; // Construire le mot clé
    ...
    .done((parsedJSON) => {
			console.log('Réponse terminée, parsedJSON:', parsedJSON);
    ...
    .fail((errorReport) => {
        // Cette fonction est appelée quand une erreur se produit.
