...
let previousTokens = ["", "", ""];
let inKey;
let prevTokenWasNewLine = false;
let nextToken = "";
let currentKeyword = "";
let buildingKeyword = ""; // Pour construire les mots clés "action" et "action_input"
...
class MoniaWebComponent extends HTMLElement {
    ...
    async loadPageContent(shadowRoot, content) {
        if (path) {
            ...
            MoniAIcon.style.cursor = "pointer";
            ...
            maxIcon.addEventListener("click", function () {
            	if (!isMaximized) { 
					openIcon.style.display = "none";
					section.style.display = "block";
					section.style.right = "7%";
					toggleButton.style.right = "7%";
			...
			const moniaLight = shadowRoot.getElementById("monia");
...
//         }
//         if (data && data.base) {
//             parentData.displayedUrl = data.displayedUrl;
//         }        
//         
//         if (data && data.title) {
...
// //                 const text = dataObject.action_input;
//                 const text = dataObject.token;
...
// 			div.innerHTML = "<br><br>Final Answer";
...
// 		"web-browser": () => {
// 			let div = document.createElement("div");
// 			div.innerHTML = "<br><br>Searching the net";
// 			answersElement.appendChild(div);
// 			currentAction = 'web-browser';
// 			console.log("Searching the net");
// 		},
// 		"company-qa": () => {
// 			let div = document.createElement("div");
// 			div.innerHTML = "<br><br>Searching in documents";
// 			answersElement.appendChild(div);
...
// 		"company-mem": () => {
// 			let div = document.createElement("div");
// 			div.innerHTML = "<br><br>Searching in memory";
// 			answersElement.appendChild(div);
...
// 				let combinedPreviousTokens = previousTokens[2] + previousTokens[1] + previousTokens[0];
// 	
// 				// Si nous construisons une valeur
// 				if (buildingValue) {
// 					tokBuffer += token;
// 
// 					// Si le buffer contient une séquence de fin de valeur
// 					if (tokBuffer.endsWith("\",\n") || tokBuffer.endsWith("\"\n}")) {
// 						processedToken = tokBuffer;
// 
// 						if (currentAction) {
// 							if (structuredResult[currentAction]) {
// 								structuredResult[currentAction].push(processedToken);
// 							} else {
// 								structuredResult[currentAction] = [processedToken];
// 							}
// 							currentAction = ""; // Reset currentAction
// 						}
// 						tokBuffer = "";  // Reset buffer
// 						buildingValue = false;
// 												
// 					} else {
// 						// Ici, nous traitons chaque caractère de la valeur dès que nous le recevons
// 
// 						// Traiter le token
...
// 
// 						// Si la combinaison des tokens précédents contient une séquence de saut de ligne
// 						if (combinedPreviousTokens.includes("\\n") && !prevTokenWasNewLine&& !nextToken.includes("\n")) {
// 			
// 							processedToken = "<br>";
// 							prevTokenWasNewLine = true;
// 						} else {
...
// 					}
// 				}
// 				// Si nous ne construisons pas une valeur
// 				else {
// 					tokBuffer += token;
// 
// 					// Construire le mot clé
// 					if (token !== '"') {
// 						buildingKeyword += token;
// 					}
// 
// 					// Si le mot clé est terminé
// 					if (token === '"') {
// 						console.log('keyword: ', buildingKeyword);
// 						const actionSequence = '"action":';
// 
// 						if (buildingKeyword.includes(actionSequence)) {
// 							console.log('action detected');
// 							currentAction = actionHandlers[tokBuffer];
// 							if (currentAction) {
// 								currentAction();
// 							}
// 						}
// 						tokBuffer = "";  // Reset buffer
// 						buildingKeyword = ""; // Reset buildingKeyword
// 						buildingValue = true;
// 					}
// 				}
// 
...
processQueue(question) {
    ...
    if (queue.length > 0) {
		...
		if (!message.includes("[DONE]")) {
			...
			const text = dataObject.token;
			 if (text === '{') {
				isInDataSegment = !isInDataSegment;  // toggle data segment flag
				if (!isInDataSegment) {
				console.log('we are exiting a data segment');
					// We are exiting a data segment
					if (currentKey !== "" && currentValue !== "") {
						console.log(`Key: ${currentKey}`);
						console.log(`Value: ${currentValue}`);
						if (actionHandlers[currentKey]) {
							actionHandlers[currentKey]();
						} else if (currentKey === "action_input") {
							console.log(`Found action_input: ${currentValue}`);
							answersElement.insertAdjacentHTML('beforeend', currentValue);
						}
					}
				...
				tokBuffer += text;
				console.log(`Processing: ${tokBuffer}`);
			
				if (tokBuffer.includes('":"') && isCollectingKey) {
				console.log('we are in data segment');
					let parts = tokBuffer.split('":"');
					currentKey = parts[0].replace(/"/g, '').trim();
					tokBuffer = parts[1].trim();
					console.log(`currentKey: ${currentKey}, tokBuffer: ${tokBuffer}`);

					isCollectingKey = false;
				}

				if ((tokBuffer.endsWith('","') || tokBuffer.endsWith('}')) && !isCollectingKey) {
					currentValue = tokBuffer.slice(0, -1).replace(/"/g, '').trim();
					tokBuffer = "";
					isCollectingKey = true;
				}
			} 
