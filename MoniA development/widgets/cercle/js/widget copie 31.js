...
// 					console.log("Inside Final Answer");
					...
					div.innerHTML = "Voilà ma réponse";
					div.classList.add("a_final");
...
// 					console.log("Token before processing: ", token);
				...
				"web-browser": () => {
					this.createToolElement("Je recherche sur internet");
					currentAction = 'web-browser';
					console.log("Searching the net");
				},
				"company-qa": () => {
					this.createToolElement("Je cherche dans les documents disponibles");
					currentAction = 'company-qa';
					console.log("Searching in documents");
				},
				"company-mem": () => {
					this.createToolElement("Bon je regarde dams mon historique");
					currentAction = 'company-mem';
					console.log("Searching in memory");
				},
			};
	
			this.createToolElement = function(content) {
				let stepsDiv = document.getElementById("steps");
				if (!stepsDiv) {
					stepsDiv = document.createElement("div");
					stepsDiv.id = "steps";
...
// 		 				console.log('buffer ',tokBuffer);
						...
						if (processedToken.includes("[")) {
							...
							accumulatedLinkText += processedToken;
							if (processedToken.includes("]")) {
								processedToken = accumulatedLinkText.replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" target="_blank">$1</a>');
								accumulatingLink = false;
								accumulatedLinkText = "";
							} else {
								processedToken = ""; // On ne l'affiche pas encore
							}
						}
